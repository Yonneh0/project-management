...
func main() {
	...
	compileCache := pkg.NewCompileCache(60 * time.Second)

	store, err := pkg.InitDatabase(dbPath, compileCache)
	if err != nil {
		log.Fatalf("database initialization failed: %v", err)
	}

	// Register global store for watcher-based cache invalidation
	pkg.SetGlobalStore(store)
	...
	watcher, err := pkg.NewFileWatcher(targetDir, dbPath, store.UpsertFile, store.DeleteFile)
	if err != nil {
	...
	tools.RegisterTools(mcpServer, store, targetDir)
